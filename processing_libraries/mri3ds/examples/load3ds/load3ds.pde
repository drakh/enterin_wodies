...
void setup()
{
  ...
  vscene = new V3dsScene( this, "sponza.3ds" );
//  vscene.useMaterial( false );
//  vscene.setGlobalColor( 1, 0, 0, 1 );

  // print out the first camera and light from the scene (debug)
...
//  camera( 0, -200, 600, 0, 0, 0, 0, 1, 0 );//cam_pos.x, cam_pos.y, cam_pos.z, cam_tar.x, cam_tar.y, cam_tar.z, cam_up.x, cam_up.y, cam_up.z );
  camera( 0, -2, 6, 0, -3*sin(time)*2, -8, 0, 1, 0 );//cam_pos.x, cam_pos.y, cam_pos.z, cam_tar.x, cam_tar.y, cam_tar.z, cam_up.x, cam_up.y, cam_up.z );
//  scale( 1, -1, 1 );
